...
static const char kChaseEdgeVS[] = "EdgeVS.hlsl";
static const char kChaseEdgePS[] = "ChaseEdgePS.hlsl";
...
void ChaseEffect::Initialize(ColorBuffer* originalTexture) {
    edgeTexture_.Create(L"chaseEdgeTexture", originalTexture->GetWidth(), originalTexture->GetHeight(), originalTexture->GetFormat());
    ...
    //Edge
    {
        ...
        edgeRootSignature_.Create(L"ChaseEdge RootSignature", rootSignatureDesc);
    }

    {
        ...
        psoDesc.pRootSignature = edgeRootSignature_;

        auto vs = shaderManager->Compile(kChaseEdgeVS, ShaderManager::kVertex);
        auto ps = shaderManager->Compile(kChaseEdgePS, ShaderManager::kPixel);
        ...
        edgePipelineState_.Create(L"ChaseEdge PSO", psoDesc);
    }
...
void ChaseEffect::EdgeRender(CommandContext& commandContext, GeometryRenderingPass& geometryRenderingPass) {
    ...
    commandContext.SetRootSignature(edgeRootSignature_);
