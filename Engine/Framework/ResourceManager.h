...
public:
    ...
    void AddTexture(const std::string& name, const std::shared_ptr<Texture>& texture) { textureMap_.emplace(std::make_pair(name, texture)); }
    std::shared_ptr<Texture> FindTexture(const std::string& name) const { return textureMap_.at(name); }

    void AddSound(const std::string& name, const std::shared_ptr<Sound>& sound) { soundMap_.emplace(std::make_pair(name, sound)); }
    std::shared_ptr<Sound> FindSound(const std::string& name) const { return soundMap_.at(name); }

    void AddAnimation(const std::string& name, const std::shared_ptr<Animation>& animation) { animationMap_.emplace(std::make_pair(name, animation)); }
